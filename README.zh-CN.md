...
标签 v1 可在便签标题下方直接编辑，并沿用普通 Draft、Revision、E2E 加密、导出与导入路径。侧边栏只有在打开筛选器时才加载标签目录。精确标签筛选可与全文搜索叠加；在标签筛选状态下新建的便签会继承当前标签，因此不会从当前上下文中消失。

置顶与排序 v1 会把常用便签保持在侧边栏顶部，其余便签可按最近修改、最早修改或标题排序。它们被明确限定为当前设备的本地工作区偏好，仅写入 SQLite，不创建 Revision、远端对象或同步流量，因此不同设备的置顶与排序状态可以不同。

回收站 v1 提供经过确认的软删除与按需恢复。已有历史的便签在删除时追加可同步的 tombstone Revision；恢复时追加包含原始内容的普通子 Revision。从未提交过的本地便签仍可恢复，但不会向远端发布没有意义的删除事件。回收站只在打开时加载，并且不提供永久删除不可变历史的入口。
